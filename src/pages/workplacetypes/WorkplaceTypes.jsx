...
// Generates a consistent color per initial letter (matches the screenshot's varied avatar colors)
const AVATAR_COLORS = [
  "bg-orange-500",
  "bg-purple-600",
  "bg-green-600",
    ...
    <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-10">
      ...
      <div className="px-6 py-5 space-y-4">
        <div className={`flex items-center gap-4 p-4 rounded-xl bg-blue-50`}>
          <div
            className={`w-14 h-14 ${avatarColor(item.name)} rounded-xl flex items-center justify-center text-white text-xl font-bold flex-shrink-0`}
          >
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-gray-50 rounded-lg p-3">
            ...
            <span
              className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium ${item.is_status ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
            >
              <span
                className={`w-1.5 h-1.5 rounded-full ${item.is_status ? "bg-green-500" : "bg-yellow-500"}`}
              />
      ...
      <div className="flex items-center gap-2 px-6 py-4 border-t border-gray-100 bg-gray-50/50 rounded-b-2xl">
        <button
          onClick={onEdit}
          className="flex-1 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2.5 rounded-lg text-sm font-medium transition-colors"
        >
          <Pencil size={14} /> Edit
        </button>
        <button
          onClick={onDelete}
          className="flex-1 flex items-center justify-center gap-2 bg-red-500 hover:bg-red-600 text-white px-4 py-2.5 rounded-lg text-sm font-medium transition-colors"
        >
          <Trash2 size={14} /> Delete
        </button>
        <button
          onClick={onClose}
          className="flex-1 border border-gray-200 hover:bg-gray-100 text-gray-600 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors"
        >
          Close
        </button>
      </div>
      ...
      <div className="px-6 py-6 flex flex-col items-center text-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            This action cannot be undone. The workplace type will be permanently
            removed.
          </p>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="mb-2 text-sm">
        ...
        <span className="text-gray-700 font-medium">Workplace Types</span>
      </div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Workplace Types</h1>
            <p className="text-sm text-gray-500">
              Manage all the workplace types used across the platform
            </p>
          </div>
        </div>
        <button
          onClick={() => navigate("/workplace-types/add")}
          className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors"
        >
          <Plus size={16} /> Add Workplace Type
        </button>
      </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 p-4 border-b border-gray-100">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search workplace type..."
                value={search}
                onChange={(e) => {
                ...
                paginated.map((item, idx) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-9 h-9 rounded-lg ${avatarColor(item.name)} flex items-center justify-center text-white font-bold text-sm flex-shrink-0`}
                        >
                          {item.name.charAt(0).toUpperCase()}
                        </div>
                        <div>
      ...
      {viewItem && (
        <ViewModal
          ...
          onEdit={() => {
            setViewItem(null);
            navigate(`/workplace-types/edit/${viewItem.id}`);
          }}
          onDelete={() => {
            setViewItem(null);
