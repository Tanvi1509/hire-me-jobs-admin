...
const EditExperienceLevels = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const [form, setForm] = useState({ name: "", min_year: "", max_year: "", is_status: true });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to update experience level");
      setSuccess(true);
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Edit Experience Level</h1>
            <p className="text-sm text-gray-500">Update the experience level details</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm max-w-lg">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="font-semibold text-gray-800">Experience Level Information</h2>
          <span className="text-xs text-gray-400 bg-gray-100 px-2 py-1 rounded-md font-medium">ID: #{id}</span>
        </div>
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="number"
                    value={form.min_year}
                    onChange={(e) => setForm((f) => ({ ...f, min_year: e.target.value }))}
                    placeholder="e.g. 0"
                    required
                    min={0}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={form.max_year}
                    onChange={(e) => setForm((f) => ({ ...f, max_year: e.target.value }))}
                    placeholder="e.g. 1"
                    required
                    min={0}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  >
                    <span className={`w-2 h-2 rounded-full ${form.is_status ? "bg-green-500" : "bg-gray-300"}`} />
                    Active
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    <span className={`w-2 h-2 rounded-full ${!form.is_status ? "bg-red-500" : "bg-gray-300"}`} />
                    Inactive
                  </button>
                </div>
              </div>
...
export default EditExperienceLevels;
