  ...
  useEffect(() => {
    const fetchLanguage = async () => {
      try {
        ...
        // Try fetching the single record directly: GET /language/:id
        try {
          ...
          if (data?.data && !Array.isArray(data.data)) {
            ...
            item = data.data.find((lang) => String(lang.id) === String(id));
          }
        } catch (innerErr) {
        ...
        if (item) {
          ...
          setApiError("Language not found");
        }
      } catch (err) {
        setApiError("Something went wrong while fetching language");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setApiError(data?.message || "Failed to update language");
      }
    } catch (err) {
      setApiError("Something went wrong while updating language");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="p-6 bg-gray-50 min-h-screen flex items-center justify-center">
        <p className="text-gray-400 text-sm">Loading language...</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Language</h1>
          <p className="text-sm text-gray-500 mt-0.5">Update language details (ID: #{id})</p>
        </div>
      </div>
