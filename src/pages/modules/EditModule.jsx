...
const API_URL = "https://hire-me-jobs.onrender.com/module";
  ...
  useEffect(() => {
    const fetchModule = async () => {
      try {
        ...
        // Expecting { data: { id, name, status, ... } } or just the object
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.message || "Failed to update module.");
      }
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input
              ...
              placeholder="e.g., HR, Finance, Inventory"
          ...
          <div className="flex gap-4 pt-3">
            <button
              type="button"
              onClick={() => navigate("/modules")}
              className="px-6 py-3 text-sm font-semibold text-gray-600 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-all shadow-sm"
            >
              Cancel
            </button>
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-6 py-3 text-sm font-semibold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 active:scale-95 transition-all shadow-md shadow-indigo-200 disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2"
            >
              {loading ? (
                <>
