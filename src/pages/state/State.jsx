...
export default function State() {
  ...
  const fetchStates = async () => {
    ...
    try {
      ...
      setError(err.message || "Something went wrong while fetching states");
    } finally {
      setLoading(false);
    }
  };
  ...
  }, [states, search, statusFilter]);

  const totalPages = Math.max(
    1,
    Math.ceil(filteredStates.length / itemsPerPage),
  );
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`${API_URL}/${deleteState.id}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error("Failed to delete state");
      setStates((prev) => prev.filter((s) => s.id !== deleteState.id));
      setToast({
        type: "success",
        message: `"${deleteState.name}" deleted successfully`,
      });
      setDeleteState(null);
    } catch (err) {
      setToast({
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6">
      ...
      <div className="text-sm text-slate-400 mb-4">
        ...
        <span className="text-slate-600 font-medium">States</span>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800">States</h1>
            <p className="text-sm text-slate-400">Manage states and regions</p>
          </div>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2">
            <h2 className="font-semibold text-slate-800">State List</h2>
                  ...
                  <tr key={s.id} className="hover:bg-slate-50/60 transition">
                      ...
                      <div className="flex items-center gap-3">
                        <div
                          className={`h-9 w-9 rounded-full flex items-center justify-center text-white font-semibold text-sm ${
                            AVATAR_COLORS[s.id % AVATAR_COLORS.length]
                          }`}
                        >
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => navigate(`/state/edit/${s.id}`)}
                          title="Edit"
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <h3 className="font-semibold text-slate-800">State Details</h3>
              <button
              ...
              <div className="flex items-center gap-3">
                <div
                  className={`h-14 w-14 rounded-full flex items-center justify-center text-white font-bold text-lg ${
                    AVATAR_COLORS[viewState.id % AVATAR_COLORS.length]
                  }`}
                >
                  {viewState.name?.charAt(0).toUpperCase()}
                </div>
                <div>
                  <p className="text-lg font-bold text-slate-800">
                    {viewState.name}
                  </p>
                  <p className="text-xs text-slate-400">ID: #{viewState.id}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div className="bg-slate-50 rounded-lg p-3">
                  ...
                  <span
                    className={`text-xs font-semibold px-2.5 py-1 rounded-full ${
                      viewState.is_trending
                        ? "bg-violet-50 text-violet-600"
                        : "bg-slate-100 text-slate-500"
                    }`}
                  >
                    {viewState.is_trending ? "Yes" : "No"}
                  </span>
                </div>
                <div className="bg-slate-50 rounded-lg p-3 col-span-2">
                  ...
                  <p className="font-medium text-slate-700">
                    {formatDate(viewState.created_at)}
                  </p>
                </div>
                <div className="bg-slate-50 rounded-lg p-3 col-span-2">
                  <p className="text-xs text-slate-400 mb-1 flex items-center gap-1">
                  ...
                  <p className="font-medium text-slate-700">
                    {formatDate(viewState.updated_at)}
                  </p>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-2 px-6 py-4 border-t border-slate-100">
              <button
                onClick={() => {
                  navigate(`/state/edit/${viewState.id}`);
                  setViewState(null);
                }}
